...
SRC_PACK='opencore-amr-0.1.3.tar.gz'

SRC_ROOT=`pwd`/opencore-amr-0.1.3
BUILD_PATH=`pwd`/build/opencore-amr
LIB_LINK_PATH=`pwd`/build
HAEDER_LINK_PATH="$LIB_LINK_PATH/include"

LIBS=('libopencore-amrwb.a' 'libopencore-amrnb.a')
...
function build_with_args() {
	...
	export CXX="$ios_dev_root/usr/bin/llvm-g++"
	#export CPP="$ios_dev_root/usr/bin/llvm-cpp-4.2"
	export CFLAGS="-arch $arch -isysroot $sys_root -I$sys_root/usr/include"
	export CXXFLAGS="$CFLAGS"
	export LDFLAGS="-arch $arch -isysroot $sys_root -L$sys_root/usr/lib -L$sys_root/usr/lib/system"
